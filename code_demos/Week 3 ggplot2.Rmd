...
Below, we create a plot named `p1` which uses a **linear model** statistical transformation to display it's `geom_smooth`. It makes an aesthetic call to `color=continent` so there are separate linear trend lines for each continent group.
...
Now let's see what happens when we change a plot's theme. ggplot2 comes with a number of built-in themes that can be easily called. Note if you haven't saved a ggplot object, `last_plot()` gives you something to work with.
