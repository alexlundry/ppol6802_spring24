...
We'll be using `gganimate` to do our animating. If you don't already have `gifski` or `av` as an installed library, you'll want to do that (these are what support the creation of GIF and movie files respectively.  Yet again, we'll be using the `gapminder` dataset.
...
3.  `ease_aes` function takes in linear as an input argument and it defines how the transition occurs from frame to frame (in this case, linear).
...
i1 <- gapminder %>%
  filter(continent == "Asia" & 
           year == 2007) %>% 
   mutate(label_text = str_c(country, ": ", lifeExp)) %>% 
...
Here's another example with a scatterplot.  Notice the mutate statement that gets us a nice label, and that it is mapped to the `text` aesthetic.
