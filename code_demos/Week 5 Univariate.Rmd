...
d1 <- read_csv("~/Dropbox/Teaching/PPOL 6802/ppol6802-spring24/datasets/global_power_plant_database_v_1_3/global_power_plant_database.csv")
```
...
The most basic visualization task you will undertake will be the visualization of just one variable - especially if it is a straightforward categorical variable. Imagine, for example, visualizing the gender distribution of a dataset. You'll have only a few different values to visualization!
...
`sapply()` is a function that applies a specific function over a vector `class()` tells us the type of variable `names()` will give us the variable names of a dataset `[]` and the brackets allows us to select specific parts of a vector

The statement below essentially says:

1)  Give me the names of all the variables in s2
2)  for each these names, check to see if the class is equal to character
3)  if it is a character, then return the name of the variabl.e
...
No suprises here. But now, let's simply take that bar chart and convert the coordinates to polar using `coord_polar`. I've also added a `theme_void` call because without it the default gives you some weird lines and shading (try it with that removed if you don't believe me).
...
The tallest bar shows that roughly 8000 observations are clustered around the lowest possible MW capacity. Moreover, the histogram shows that there is not a ton of variability here. High kurtosis (mode is more common than it would be in a normal distribution). This is likely a function of how many different types of power plants there are in the dataset, covering everything from very tiny solar arrays to massive nuclear stations. Let's get something more useful by filtering to a specific type of fuel:
...
Looking for something nerdier? Something a little more information dense? Boxplots are odd, but can be incredibly useful (invented by Tukey, btw). Here's a great description of how they work. The box itself represents the interquartile range, running from the 25th %tile to the 75th %tile, with the bar in the middle representing the median. The whiskers extend out to the farthest non-outlier point. And outliers are shown as dots.
...
Finally, dotplots are sort of the hybrid between a waffle chart and a histogram, in which each dot has spot on the X axis designated by numeric data, and the dots are stacked, with each dot representing one observation.
